...
%An approximately $340$~deg$^{2}$ filed of view (FOV) close to the galactic plane resulted in detections of $1.5$~billion stars.  Once stars were ranked in order of decreasing variability, a subset of $315,992$
%Of these, 315Ranking observed stars by decreasing variability caused 
...
%Implementation of FLS and related analysis techniques allowed for the classification of 
%Establishing a relationship between the shapes of RR Lyrae light curves and their physical properties will allow for the determination of RR Lyrae physical properties, based solely on light curve measurements.
...
%light curve based measurement of physical properties intrinsic to RR Lyraes.
\end{abstract}
...
Observation procedures are discussed in~$\S~2.2$.  %\cref{sec:data}.
...
\iffalse
of all 1.5 billion stars in our FOV we scanned the most variable $(\log{Pr(rnd)}<=45)$\\
...
The particular sorting algorithm, written by J. Tonry~\cite{gri}, is discussed in~$\S 7.3$.  %~\cref{sec:HPS}.  
Grouping data 
...
as described in~$\S 2.3$.  %\cref{sec:cuts}.  
...
%The three values given as decimals$HPS_{completeness}$, Prob(RR), and Prob(notRR)) represent fractional percentages. 
...
%we need spectra of the RR Lyrae stars to calibrate the statistics. 
